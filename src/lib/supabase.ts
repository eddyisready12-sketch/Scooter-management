...
function normalizeSupplier(row: Record<string, unknown>): Supplier {
  return {
    ...
    euResponsiblePersonId: row.euResponsiblePersonId ? String(row.euResponsiblePersonId) : row.eu_responsible_person_id ? String(row.eu_responsible_person_id) : undefined,
    ...
    supplierSchemaVersion: Number(row.supplierSchemaVersion ?? row.supplier_schema_version ?? 1),
    herkomst: (row.herkomst ? String(row.herkomst) : undefined) as Supplier['herkomst'],
    docStatus: (row.docStatus ?? row.doc_status ?? undefined) as Supplier['docStatus'],
    packagingProfile: (row.packagingProfile ?? row.packaging_profile ?? undefined) as Supplier['packagingProfile'],
    packagingItems: (row.packagingItems ?? row.packaging_items ?? undefined) as Supplier['packagingItems'],
...
function supplierToDatabase(supplier: Supplier) {
  return {
    ...
    supplier_schema_version: supplier.supplierSchemaVersion,
    herkomst: supplier.herkomst,
    doc_status: supplier.docStatus,
    packaging_profile: supplier.packagingProfile,
    packaging_items: supplier.packagingItems,
...
function supplierToLegacyDatabase(supplier: Supplier) {
  return {
    ...
    city: supplier.city,
    country: supplier.country,
    supplierSchemaVersion: supplier.supplierSchemaVersion,
    herkomst: supplier.herkomst,
    docStatus: supplier.docStatus,
    packagingProfile: supplier.packagingProfile,
    packagingItems: supplier.packagingItems,
    isPackagingSupplier: supplier.isPackagingSupplier ?? false,
    packagingMaterials: supplier.packagingMaterials,
    ppwrSupplierRole: supplier.ppwrSupplierRole,
